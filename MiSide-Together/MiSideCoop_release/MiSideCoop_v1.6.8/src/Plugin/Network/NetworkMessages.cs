...
namespace MiSideCoop.Network
{
    ...
    public enum MsgId : byte
    {
        ...
        HandItem      = 10,  // v1.6.8 — synchronise l'activation des hand-items 1ère/3ème personne
    }
    ...
    // ── v1.6.8 — Synchronisation d'activation des hand-items ─────────────────
    // Émis par le sender quand un hand-item (Smartphone, Knife, Tetris…) bascule
    // localement actif/inactif. Le receiver applique le même état à :
    //   • sa copie locale du 1ère-personne (même path → "Player/RightItem FixPosition/{ItemName}")
    //   • le 3rd-person Right item sur le clone 3D représentant le sender
    // De cette façon les DEUX joueurs voient l'item dans leurs propres mains ET
    // sur le clone du peer, conformément à l'attente utilisateur v1.6.8.
    public struct HandItemMessage : INetMessage
    {
        public string ItemName;   // nom court (ex: "Smartphone", "Knife", "Tetris")
        public bool   IsActive;
        public string Slot;       // "FP" (first-person) ou "TP" (third-person right hand)

        public MsgId Id => MsgId.HandItem;

        public void Write(BinaryWriter w)
        {
            w.Write(ItemName ?? string.Empty);
            w.Write(IsActive);
            w.Write(Slot ?? string.Empty);
        }
        public void Read(BinaryReader r)
        {
