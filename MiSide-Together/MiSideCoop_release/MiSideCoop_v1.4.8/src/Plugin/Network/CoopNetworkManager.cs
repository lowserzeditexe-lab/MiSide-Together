    ...
    public class CoopNetworkManager : MonoBehaviour
    {
        ...
        private void SpawnPlayer2Remote()
        {
            try
            {
                ...
                var go = cloned ?? CreateDefaultHumanoid("Player2_Guest");
                _player2IsSynthetic = (cloned == null);
                ...
                // v1.4.8 — ApplySkinColor uniquement pour fallback synthétique.
                // Le clone 3D garde ses textures MiSide originales (sinon il est
                // entièrement repeint en bleu uniforme = "bonhomme bleu").
                if (_player2IsSynthetic)
                    _player2.ApplySkinColor(MiSideCoopPlugin.Player2SkinColor.Value);
                MiSideCoopPlugin.Logger.LogInfo(
                    $"[Co-op] Guest avatar (Player2) spawned on host "
                  + $"({(_player2IsSynthetic ? "synthetic" : "3D clone")}).");
            }
            catch (Exception ex)
            {
        ...
        private void SpawnPlayer1Remote()
        {
            ...
            try
            {
                ...
                var go = cloned ?? CreateDefaultHumanoid("Player1_Host_Remote");
                _player1IsSynthetic = (cloned == null);
