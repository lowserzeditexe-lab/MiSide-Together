...
namespace MiSideCoop.UI
{
    ...
    public class CoopMenuUI : MonoBehaviour
    {
        ...
        /// <summary>
        /// Cherche une Font via les Text existants dans la scène ou dans la
        /// hiérarchie de la caméra principale. CHAQUE API Unity est isolée
        /// dans sa propre micro-méthode pour qu'un échec JIT (méthode strippée)
        /// soit attrapable par un try/catch d'appelant — sans cette isolation,
        /// l'exception remonte avant que le try interne ne puisse l'attraper.
        /// </summary>
        private static Font TryFindFontInScene()
        {
            // Voie A : énumération des racines de scène
            var roots = SafeGetRootGameObjects();
            if (roots != null)
            {
                for (int i = 0; i < roots.Length; i++)
                {
                    var f = ScanTransformForFont(roots[i] != null ? roots[i].transform : null, 0);
                    if (f != null) return f;
                }
            }

            // Voie B : depuis Camera.main.transform.root (utile si voie A strippée)
            var camRoot = SafeGetMainCameraRoot();
            if (camRoot != null)
            {
                var f = ScanTransformForFont(camRoot, 0);
                if (f != null) return f;
            }
        ...
        /// <summary>Appel isolé — si Scene.GetRootGameObjects() est strippé, l'exception
        /// JIT remonte ici et est gérée par le try/catch dans Update().</summary>
        private static GameObject[] SafeGetRootGameObjects()
        {
            try
            {
