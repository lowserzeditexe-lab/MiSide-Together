# MiSide Co-op Mod v1.7.0

Mod BepInEx 6 (IL2CPP) ajoutant un **mode coopératif en ligne pour 2 joueurs** au jeu Unity *MiSide*.

## 🆕 What's new in v1.7.0

- **Fix critique IL2CPP animator filter** : `runtimeAnimatorController` est strippé en IL2CPP MiSide et retourne `null` même sur un Animator valide. Le filtre `hasCtrl` excluait donc TOUS les Animators et faisait sélectionner `Player Arms` (pc=0) au lieu de `Person` (pc=15) → ghost en T-pose et spam de `NullReferenceException`. Critère désormais : `parameterCount > 0` uniquement.
- Patches appliqués à `PlayerAvatar.ResolveBestAnimator`, `CoopNetworkManager.ResolvePersonAnimator` et `RealMcCloner.PickBestAnimatorForEnum`.
- `HandItemSync` et `SharedPovController` correctement enregistrés via `ClassInjector.RegisterTypeInIl2Cpp<…>()`.
