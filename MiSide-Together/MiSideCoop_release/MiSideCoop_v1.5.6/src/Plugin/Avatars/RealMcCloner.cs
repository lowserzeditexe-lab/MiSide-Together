...
namespace MiSideCoop.Avatars
{
    ...
    internal static class RealMcCloner
    {
        ...
        {
            // v1.5.6 — STRIP DES OBJETS "EN MAIN" DU MC (gameboy, téléphone, etc.)
            //
            // Le clone "Person" contient des sous-GO pour les items que le MC tient
            // en main (cf. screenshot v1.5.5 : la GameBoy reste affichée dans la
            // main du ghost). Ces objets ont des noms reconnaissables — on les
            // SetActive(false) pour ne pas les voir sur le ghost. Le ghost ne
            // doit montrer que le body + vêtements + tête, pas les items qui
            // appartiennent au gameplay local du MC.
            //
            // Liste de keywords défensive — on log AUSSI tous les noms enfants
            // pour pouvoir étendre cette liste version après version sans
            // dépendre du F10 dump.
            try
            {
                ...
                ForEachChildTransform(root.transform, child =>
                {
                    ...
                    if (child == root.transform) return; // jamais le root
                        ...
                        if (lo.Contains(kw))
                        {
                            try
                            {
                                if (child.gameObject.activeSelf)
                                {
                                    ...
                                    strippedItems++;
                                    MiSideCoopPlugin.Logger?.LogInfo(
                                        $"[Co-op]   StripHandItems: SetActive(false) on '{n}' "
                                      + $"(matched keyword '{kw}').");
                                }
                            }
                            catch { }
                ...
                MiSideCoopPlugin.Logger?.LogInfo(
                  ...
                  + "(GameBoy, phone, tetris, etc.).");
            }
            catch (Exception ex)
            {
                MiSideCoopPlugin.Logger?.LogWarning(
                    $"[Co-op] RealMcCloner.StripHandItems: {ex.Message}");
            }
