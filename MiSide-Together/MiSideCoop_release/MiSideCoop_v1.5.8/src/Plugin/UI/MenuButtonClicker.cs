...
namespace MiSideCoop.UI
{
    ...
    internal static class MenuButtonClicker
    {
        ...
        private static MethodInfo _gameObjectGetComponentGenericDef;    // GameObject.GetComponent<T>() (générique) — v1.4.7
        ...
        private static void ResolveReflectionOnce()
        {
            ...
            try
            {
                ...
                // 4) v1.4.7 — Résout aussi GameObject.GetComponent<T>() (générique
                //    sans args) qui retourne un wrapper IL2CPP strongly-typed
                //    (vs Il2CppSystem.Object via la version non-générique). C'est
                //    le seul moyen d'obtenir une instance que MethodInfo.Invoke
                //    accepte sans "Object does not match target type".
                _gameObjectGetComponentGenericDef = typeof(GameObject)
                    .GetMethods(BindingFlags.Public | BindingFlags.Instance)
                    .FirstOrDefault(m => m.Name == "GetComponent"
                                      && m.IsGenericMethodDefinition
                                      && m.GetParameters().Length == 0);
                if (_gameObjectGetComponentGenericDef != null)
                {
                    MiSideCoopPlugin.Logger?.LogInfo(
                        "[Co-op] MenuButtonClicker: GameObject.GetComponent<T>() generic method resolved.");
                }
            }
            catch (Exception ex)
            {
        ...
        private static bool TryInvokeViaUnityEvent(object menuInstance, string ownerName)
        {
            try
            {
                ...
                MiSideCoopPlugin.Logger?.LogInfo(
                    $"[Co-op] MenuButtonClicker.UnityEvent: runtime type = '{runtimeType.FullName}'.");
                var field = runtimeType.GetField("eventStartNewGame",
                    BindingFlags.Public | BindingFlags.Instance);
                if (field == null)
                {
                    MiSideCoopPlugin.Logger?.LogWarning(
                        "[Co-op] MenuButtonClicker.UnityEvent: 'eventStartNewGame' field not found on runtime type.");
                    return false;
                }
                ...
                {
                    MiSideCoopPlugin.Logger?.LogWarning(
                        "[Co-op] MenuButtonClicker.UnityEvent: eventStartNewGame field is null on instance.");
                    return false;
                }
                ...
                {
                    MiSideCoopPlugin.Logger?.LogWarning(
                        $"[Co-op] MenuButtonClicker.UnityEvent: Invoke() method not found on type '{unityEvent.GetType().FullName}'.");
                    return false;
                }
        ...
        {
            // v1.4.7 — PRIORITÉ : GetComponent<T>() générique qui retourne un
            // wrapper strongly-typed Menu, indispensable pour que les Invoke
            // ultérieurs (MethodInfo.Invoke, MethodDelegate, etc.) acceptent
            // l'instance comme "of type Menu".
            if (_gameObjectGetComponentGenericDef != null)
            {
                try
                {
                    var typedGet = _gameObjectGetComponentGenericDef.MakeGenericMethod(_menuType);
                    var typed = typedGet.Invoke(t.gameObject, null);
                    if (typed != null) return typed;
                }
                catch (Exception ex)
                {
                    MiSideCoopPlugin.Logger?.LogWarning(
                        $"[Co-op] MenuButtonClicker.TryGetMenuComponent generic path: {ex.Message}");
                }
            }

            // FALLBACK : GetComponent(Il2CppSystem.Type) — retourne Il2CppSystem.Object
            // (pas strongly-typed) mais permet au moins de DÉTECTER la présence
            // du composant Menu. Si on en trouve un par ce path mais que les
            // Invoke échouent ensuite, c'est exactement le mismatch v1.4.5/6.
            try
            {
