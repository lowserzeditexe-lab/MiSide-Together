...
namespace MiSideCoop.UI
{
    ...
    public class CoopMenuUI : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            // Police via Font.CreateDynamicFontFromOSFont — isolé pour qu'un échec
            // n'empêche pas la construction du Canvas.
            try { _font = TryCreateOSFont(); }
            catch (Exception ex) { MiSideCoopPlugin.Logger?.LogWarning($"[Co-op] Font OS skip : {ex.Message}"); }

            try
            {
                BuildCanvas();
                ShowState(MenuState.Main);
                MiSideCoopPlugin.Logger?.LogInfo($"[Co-op] UI uGUI construite (font={(_font != null ? _font.name : "null")}).");
            }
            catch (Exception ex)
            {
        ...
        // ─────────────────────────────────────────────────────────────────────
        // Font helper — isolé pour éviter qu'un échec IL2CPP bloque la classe
        // ─────────────────────────────────────────────────────────────────────
