...
namespace MiSideCoop.Network
{
    ...
    public class SharedPovController : MonoBehaviour
    {
        ...
        private void TickSmartphoneWatcher()
        {
            try
            {
                // Côté client : on ne déclenche jamais le HOLDER nous-mêmes ;
                // on attend le SharedPovMessage du host qui nous fera VIEWER.
                var nm = CoopNetworkManager.Instance;
                if (nm == null || !nm.IsConnected || !nm.IsHost) return;

                // Refresh la référence cachée périodiquement (les scenes peuvent
                // recréer le Smartphone, et il n'existe que dans certaines scenes).
                _watchedSmartphoneScanTimer -= Time.deltaTime;
                if (_watchedSmartphone == null || _watchedSmartphoneScanTimer <= 0f)
                {
                    _watchedSmartphone = SafeFind(SmartphonePath);
                    _watchedSmartphoneScanTimer = WatchedScanInterval;
                    if (_watchedSmartphone == null)
                    {
                        // Pas de smartphone dans la scene actuelle : reset le flag
                        _watchedSmartphoneWasActive = false;
                        return;
                    }
                }

                bool isActive = _watchedSmartphone.activeInHierarchy;

                // Transition inactive → active : on devient HOLDER (si pas déjà actif)
                if (isActive && !_watchedSmartphoneWasActive && !IsActive)
                {
                    MiSideCoopPlugin.Logger?.LogInfo(
                        "[SharedPov] Smartphone activation detected (host) → entering HOLDER mode.");
                    StartAsHolder();
                }

                _watchedSmartphoneWasActive = isActive;
            }
            catch (Exception ex)
            {
                // Pas de spam dans la log, juste un warning ponctuel
                if (Time.frameCount % 600 == 0)
                {
                    MiSideCoopPlugin.Logger?.LogWarning(
                        $"[SharedPov] TickSmartphoneWatcher: {ex.Message}");
                }
            }
        }
