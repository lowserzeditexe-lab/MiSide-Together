...
namespace MiSideCoop.Avatars
{
    ...
    internal static class SceneDiagnostics
    {
        ...
        /// <summary>
        /// v1.5.2 — Dump hiérarchique exhaustif du GameObject MC actuel.
        /// Liste pour chaque descendant : path, activeSelf, components attachés
        /// (avec types courts), enabled state pour Renderer/Behaviour, et
        /// les paramètres + states de l'Animator s'il existe.
        ///
        /// Permet d'identifier précisément :
        ///   • Quels GameObjects sont SetActive(false) (cause du "pas de tête")
        ///   • Quels SkinnedMeshRenderer / MeshRenderer sont disabled
        ///   • Quel AnimatorController est utilisé + ses states
        ///   • La structure réelle vs ce que le dump IL2CPP suggère
        /// </summary>
        public static void DumpMcHierarchy()
        {
            var log = MiSideCoopPlugin.Logger;
            if (log == null) return;
            ...
            try
            {
                log.LogMessage("[Co-op DIAG] ═══ MC Hierarchy dump (F10) ═══");

                var mc = FindMcHeuristic();
                if (mc == null)
                {
                    log.LogWarning("[Co-op DIAG] No MC found (probably still in menu — press F10 in-game).");
                    return;
                }
                log.LogMessage($"[Co-op DIAG] MC root: '{mc.name}' (activeInHierarchy={mc.activeInHierarchy}, "
                             + $"layer={mc.layer}, tag='{mc.tag}')");

                var sb = new StringBuilder();
                int nodeCount = 0, inactiveCount = 0, renderersCount = 0, disabledRenderersCount = 0;
                DumpTransform(mc.transform, sb, 0, ref nodeCount, ref inactiveCount,
                              ref renderersCount, ref disabledRenderersCount);

                // BepInEx tronque les logs > ~4KB. On split en blocs de 3000 chars.
                var full = sb.ToString();
                const int CHUNK = 3000;
                for (int i = 0; i < full.Length; i += CHUNK)
                {
                    var len = Math.Min(CHUNK, full.Length - i);
                    log.LogMessage(full.Substring(i, len));
                }

                log.LogMessage($"[Co-op DIAG] Hierarchy stats: {nodeCount} nodes, "
                             + $"{inactiveCount} inactive, {renderersCount} renderers "
                             + $"({disabledRenderersCount} disabled).");

                // Animator details
                try
                {
                    var anim = mc.GetComponent<Animator>() ?? mc.transform.GetComponentInChildrenSafe<Animator>();
                    if (anim != null)
                    {
                        log.LogMessage($"[Co-op DIAG] Animator on '{anim.gameObject.name}': "
                                     + $"controller='{(anim.runtimeAnimatorController != null ? anim.runtimeAnimatorController.name : "null")}', "
                                     + $"layerCount={anim.layerCount}, speed={anim.speed}, enabled={anim.enabled}.");
                        try
                        {
                            for (int li = 0; li < anim.layerCount; li++)
                            {
                                var info = anim.GetCurrentAnimatorStateInfo(li);
                                log.LogMessage($"[Co-op DIAG]   Layer {li}: stateHash={info.fullPathHash}, "
                                             + $"normTime={info.normalizedTime:F2}, length={info.length:F2}.");
                            }
                        }
                        catch (Exception ex)
                        {
                            log.LogWarning($"[Co-op DIAG] Animator state enum failed: {ex.Message}");
                        }
                    }
                    else
                    {
                        log.LogWarning("[Co-op DIAG] No Animator found on MC or descendants.");
                    }
                }
                catch (Exception ex)
                {
                    log.LogWarning($"[Co-op DIAG] Animator inspection failed: {ex.Message}");
                }

                log.LogMessage("[Co-op DIAG] ═══ end MC hierarchy dump ═══");
            }
            catch (Exception ex)
            {
                log.LogError($"[Co-op DIAG] DumpMcHierarchy failed: {ex}");
            }
        }

        private static void DumpTransform(Transform t, StringBuilder sb, int depth,
            ref int nodeCount, ref int inactiveCount,
            ref int renderersCount, ref int disabledRenderersCount)
        {
            if (t == null || depth > 12) return;
            nodeCount++;
            var go = t.gameObject;
            if (!go.activeSelf) inactiveCount++;
            ...
            try
            {
                ...
                {
                    renderersCount++;
                    if (!smr.enabled) disabledRenderersCount++;
                    renderStr += $" [SMR:{(smr.enabled ? "on" : "OFF")}]";
                }
                var mr = t.GetComponent<MeshRenderer>();
                if (mr != null)
                {
                    renderersCount++;
                    if (!mr.enabled) disabledRenderersCount++;
                    renderStr += $" [MR:{(mr.enabled ? "on" : "OFF")}]";
                }
                var anim = t.GetComponent<Animator>();
                if (anim != null) renderStr += " [Anim]";
                var cc = t.GetComponent<CharacterController>();
                if (cc != null) renderStr += " [CC]";
            }
            catch { }

            var activeMarker = go.activeSelf ? "✓" : "✗";
            sb.AppendLine($"{indent}{activeMarker} {go.name}{renderStr} (layer={go.layer})");
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                DumpTransform(c, sb, depth + 1, ref nodeCount, ref inactiveCount,
                              ref renderersCount, ref disabledRenderersCount);
            }
        }
