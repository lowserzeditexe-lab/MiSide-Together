...
namespace MiSideCoop.UI
{
    ...
    public class CoopMenuUI : MonoBehaviour
    {
        ...
        /// <summary>
        /// Police partagée trouvée par la dernière instance de <c>CoopMenuUI</c>.
        /// Permet aux autres modules (ex: <c>AutoUpdater</c>) d'utiliser la même
        /// police MiSide custom (Oswald-Bold) au lieu de retomber sur Arial.
        /// </summary>
        public static Font SharedFont { get; private set; }
            ...
            if (_fontSearchTries < 30)
            {
                ...
                if (found != null)
                {
                    ...
                    if (isUpgrade)
                    {
                        ...
                        _font = found;
                        SharedFont = found; // partage avec AutoUpdater & autres modules
        ...
        private static Font SafeCreateOSFont(string osName)
        {
            ...
            {
                // 1ère exception → on log (en LogInfo, pas Warning : la cascade
                // de fallback fonctionne très bien, c'est purement informatif)
                // puis on désactive la cascade entière pour ne pas multiplier
                // les exceptions identiques.
                _osFontApiBroken = true;
                MiSideCoopPlugin.Logger?.LogInfo(
                    $"[Co-op] OS font API stripped ({ex.GetType().Name}). " +
                    $"Will try to use the game's own font from the scene.");
                return null;
            }
        }
        ...
        private void CreateRoom()
        {
            ...
            SetStatus(_createStatusText, "Connecting to relay…", TXT_DIM);

            var room = GetOrAddRoomManager();
            string code = room.GenerateRoomCode();
            _roomCode = code;
            ...
            SetStatus(_createStatusText,
                "Room created. Share the code and wait for guest…",
                TXT_OK);
        }
        ...
        private void JoinRoom()
        {
            ...
            SetStatus(_joinStatusText, $"Joining room {code}…", TXT_DIM);

            CoopNetworkManager.Instance?.StartClient(code);
