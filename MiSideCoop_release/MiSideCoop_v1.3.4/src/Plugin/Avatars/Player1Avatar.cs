...
namespace MiSideCoop.Avatars
{
    ...
    public class Player1Avatar : PlayerAvatar
    {
        ...
        public override void Initialize(string playerName, bool isLocal)
        {
            ...
            // Voir Player2Avatar.Initialize : on enveloppe pour éviter qu'une
            // MissingMethodException IL2CPP (méthode générique strippée) ne
            // casse SpawnPlayer1Local/Remote et le pipeline de connexion.
            try
            {
            ...
            catch (Exception ex)
            {
                MiSideCoopPlugin.Logger?.LogError(
                    $"[Co-op] Player1Avatar.Initialize('{playerName}', local={isLocal}) failed: {ex.Message}\n{ex.StackTrace}");
            }
        }
