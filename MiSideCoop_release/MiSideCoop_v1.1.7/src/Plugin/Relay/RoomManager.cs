...
namespace MiSideCoop.Relay
{
    ...
    public class RoomManager : MonoBehaviour
    {
        ...
        private IEnumerator RegisterCoroutine(string code, System.Action<bool> callback)
        {
            ...
            // IL2CPP-safe : v1.1.6 a montré que TOUTES les classes d'upload
            // sont strippées dans MiSide (UploadHandlerRaw ctor, ctor à 2 args
            // de UnityWebRequest, factory Put). Seul UnityWebRequest.Get(string)
            // survit (factory utilisée par le jeu pour charger ses bundles).
            // On utilise donc l'endpoint GET /register/:code/:port côté relais
            // (alias ajouté en v1.1.7). Pas de body, pas d'upload handler.
            using var req = UnityWebRequest.Get(
                $"{relay}/register/{code.ToUpper()}/{port}");
            req.timeout = 10;
