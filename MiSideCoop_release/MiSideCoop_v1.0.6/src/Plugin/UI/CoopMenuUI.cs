...
namespace MiSideCoop.UI
{
    ...
    public class CoopMenuUI : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            // Chaque étape est isolée : un échec IL2CPP sur l'une n'empêche pas les autres.
            try { EnsureEventSystem(); }
            catch (Exception ex) { MiSideCoopPlugin.Logger?.LogWarning($"[Co-op] EventSystem skip : {ex.Message}"); }

            try { _font = FindFont(); }
            catch (Exception ex) { MiSideCoopPlugin.Logger?.LogWarning($"[Co-op] Font skip : {ex.Message}"); }
        ...
        private void Update()
        {
            // Polling des inputs (UnityAction<string> strippé en IL2CPP)
            PollInputs();
        ...
        /// <summary>
        /// Lit les valeurs des InputField en polling (UnityAction&lt;string&gt; est strippé
        /// en IL2CPP, donc on ne peut pas brancher .onValueChanged.AddListener).
        /// </summary>
        private void PollInputs()
        {
            ...
            if (_joinCodeInput != null)
            {
                ...
                if (upper != v)
                {
                    _joinCodeInput.text = upper; // force majuscules
                }
                if (_joinSubmitButton != null)
                {
                    bool ok = upper.Length == 6 && !_isConnecting;
                    if (_joinSubmitButton.interactable != ok)
                        _joinSubmitButton.interactable = ok;
                }
            }
        }
        ...
        private GameObject BuildPanelMain(GameObject parent)
        {
            ...
            // Note IL2CPP : on N'UTILISE PAS onValueChanged.AddListener (UnityAction<T> strippé).
            // On lira _playerNameInput.text directement à la sauvegarde.
        ...
        private GameObject BuildPanelJoin(GameObject parent)
        {
            ...
            // Note IL2CPP : UnityAction<string> strippé → pas d'onValueChanged.
            // La conversion en majuscules + activation du bouton sont gérées en polling
            // dans Update() (méthode PollJoinInput).
        ...
        {
            // Lecture directe (le polling onValueChanged étant strippé en IL2CPP)
        ...
        {
            // Lecture directe depuis l'InputField (UnityAction<string> strippé en IL2CPP)
        ...
        /// <summary>
        /// Trouve une police utilisable.
        /// Toutes les API de recherche sont strippées en IL2CPP (FindObjectsOfTypeAll,
        /// FindObjectOfType, génériques et non-génériques). On utilise donc
        /// Font.CreateDynamicFontFromOSFont qui charge une police système Windows.
        /// </summary>
        private static Font FindFont()
        {
            // ① Tentative : Resources.FindObjectsOfTypeAll(Type) — strippé sur MiSide
            try
            {
                var objs = Resources.FindObjectsOfTypeAll(typeof(Font));
                if (objs != null)
                {
                    for (int i = 0; i < objs.Length; i++)
                    {
                        var f = objs[i] as Font;
                        if (f != null) return f;
                    }
                }
            }
            ...
            // ② Tentative : police système Windows via OSFont
            try
            {
                var osFont = Font.CreateDynamicFontFromOSFont(
                    new[] { "Arial", "Verdana", "Tahoma", "Segoe UI", "Helvetica" }, 16);
                if (osFont != null) return osFont;
            }
            catch (Exception ex)
            {
                MiSideCoopPlugin.Logger?.LogWarning($"[Co-op] CreateDynamicFontFromOSFont : {ex.Message}");
            }

            // ③ Tentative : ressource built-in (souvent strippée)
            try
            {
                var builtin = Resources.GetBuiltinResource(typeof(Font), "Arial.ttf") as Font;
                if (builtin != null) return builtin;
            }
            catch { /* dernière chance épuisée */ }
        ...
        private static void EnsureEventSystem()
        {
            try
            {
                ...
                // Recherche via Resources (Object.FindObjectOfType est strippé en IL2CPP)
                var existing = Resources.FindObjectsOfTypeAll(typeof(EventSystem));
                if (existing != null && existing.Length > 0)
                {
                    for (int i = 0; i < existing.Length; i++)
                    {
                        if (existing[i] != null) return; // un EventSystem existe déjà
                    }
                }
            ...
            {
                // NE PAS propager : le jeu a probablement déjà un EventSystem (il utilise uGUI).
                MiSideCoopPlugin.Logger?.LogWarning(
                    $"[Co-op] EnsureEventSystem ignoré (IL2CPP) : {ex.Message}");
            }
        }
    }
}
