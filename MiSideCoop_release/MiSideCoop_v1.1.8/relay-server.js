...
const server = http.createServer(async (req, res) => {
  ...
  // ── GET /register/:code/:port ──────────────────────────────────────────────
  // Alias IL2CPP-safe : Unity MiSide a TOUTES ses classes d'upload strippées
  // (UploadHandlerRaw ctor, UnityWebRequest..ctor(string,string), Put). Seul
  // UnityWebRequest.Get(string) survit → le client utilise donc GET pour
  // enregistrer une room. Les données (code + port) tiennent dans l'URL.
  const registerGetMatch = url.match(/^\/register\/([a-z0-9]{6})\/(\d{1,5})$/);
  if (method === 'GET' && registerGetMatch) {
    const key  = registerGetMatch[1].toUpperCase();
    const port = parseInt(registerGetMatch[2], 10);
    if (port < 1 || port > 65535) {
      return send(res, 400, { error: 'Port invalide (1–65535).' });
    }
    const ip = getClientIp(req);
    rooms.set(key, { ip, port, created: Date.now() });
    console.log(`[Relay] Room enregistrée (GET) : ${key} → ${ip}:${port}`);
    return send(res, 200, { success: true, code: key, ip, port });
  }
  ...
  // ── GET /room/:code/delete ─────────────────────────────────────────────────
  // Alias IL2CPP-safe pour suppression de room (v1.1.8+). Voir explication
  // dans /register/:code/:port — MiSide a strippé l'API UnityWebRequest, le
  // client utilise donc HttpClient .NET avec uniquement des GET.
  const deleteGetMatch = url.match(/^\/room\/([a-z0-9]{6})\/delete$/);
  if (method === 'GET' && deleteGetMatch) {
    const key     = deleteGetMatch[1].toUpperCase();
    const existed = rooms.delete(key);
    if (!existed) return send(res, 404, { error: `Room '${key}' introuvable.` });

    console.log(`[Relay] Room supprimée (GET) : ${key}`);
    return send(res, 200, { success: true });
  }
...
server.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`   GET    /register/:code/:port  (alias IL2CPP-safe)`);
  console.log(`   GET    /resolve/:code`);
  console.log(`   DELETE /room/:code`);
  console.log(`   GET    /room/:code/delete  (alias IL2CPP-safe)`);
