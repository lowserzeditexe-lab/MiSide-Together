        ...
        public static bool ClickNewGame()
        {
            try
            {
                    ...
                    && _il2cppTypeOfGenericDef != null && _gameObjectGetComponentByIl2CppType != null)
                {
                    ...
                    {
                        // v1.4.5 — Re-résoudre la méthode sur le TYPE RUNTIME de
                        // l'instance pour éviter "Object does not match target type".
                        //
                        // En IL2CPP, AccessTools.TypeByName("Menu") retourne le
                        // type wrapper Il2CppInterop, mais GetComponent renvoie une
                        // instance dont le runtime type est un proxy légèrement
                        // différent (mêmes méthodes mais DeclaringType distinct).
                        // MethodInfo.Invoke vérifie strictement IsInstanceOfType
                        // contre DeclaringType → rejet si on utilise la MethodInfo
                        // pré-résolue depuis _menuType. La parade : récupérer la
                        // MethodInfo via menuInstance.GetType() — DeclaringType
                        // matche alors forcément l'instance.
                        MethodInfo runtimeMethod = null;
                        try
                        {
                            var runtimeType = menuInstance.GetType();
                            runtimeMethod = runtimeType.GetMethod(
                                "ButtonNewGame",
                                BindingFlags.Public | BindingFlags.Instance);
                        }
                        catch (Exception ex)
                        {
                            MiSideCoopPlugin.Logger?.LogWarning(
                                $"[Co-op] MenuButtonClicker: runtime type method lookup failed: {ex.Message}");
                        }

                        // Premier essai : méthode résolue sur le type runtime.
                        // Fallback : la méthode pré-résolue sur _menuType (au cas
                        // où une future version IL2CPP corrigerait le mismatch).
                        ...
                        try
                        {
                            ...
                            MiSideCoopPlugin.Logger?.LogInfo(
                              ...
                              + $"(via {(runtimeMethod != null ? "runtime-type" : "cached")} method).");
                            return true;
                        }
                        catch (Exception ex)
                        {
