...
using System.Text;
using BepInEx.Unity.IL2CPP.Utils;   // v1.1.4 : MonoBehaviourExtensions.StartCoroutine
                                     //          (static) → convertit System.Collections.IEnumerator
                                     //          en Il2CppSystem.Collections.IEnumerator.
                                     //          On DOIT appeler via la classe statique
                                     //          MonoBehaviourExtensions.StartCoroutine(this, ...)
                                     //          car l'instance method UnityEngine.MonoBehaviour
                                     //          .StartCoroutine(IEnumerator) existe au compile
                                     //          (référence) mais est STRIPPÉE en IL2CPP MiSide
                                     //          → MissingMethodException sinon.
using UnityEngine;
...
namespace MiSideCoop.Relay
{
    ...
    public class RoomManager : MonoBehaviour
    {
        ...
        //
        // IMPORTANT IL2CPP : on appelle MonoBehaviourExtensions.StartCoroutine
        // EXPLICITEMENT (et non `this.StartCoroutine(...)`), car la résolution
        // C# privilégie toujours la méthode d'instance UnityEngine.MonoBehaviour
        // .StartCoroutine(System.Collections.IEnumerator) qui existe dans la
        // référence mais est STRIPPÉE par le stripping IL2CPP de MiSide.
        // L'appel explicite force l'utilisation de l'extension BepInEx qui
        // convertit l'IEnumerator managé en Il2CppSystem.Collections.IEnumerator.
        ...
        private IEnumerator RegisterCoroutine(string code, System.Action<bool> callback)
        {
            ...
            // IL2CPP-safe POST : v1.1.5 a constaté que UnityWebRequest.Put
            // (string, byte[]) est ÉGALEMENT strippé dans MiSide. Seules les
            // factories effectivement utilisées par le jeu survivent —
            // UnityWebRequest.Get(string) a déjà fonctionné dans
            // ResolveCoroutine (cf. logs v1.1.4+). On crée donc la requête
            // via Get() puis on bascule en POST en mutant les propriétés
            // (les setters method/uploadHandler/downloadHandler/timeout font
            // partie du noyau UnityWebRequest et sont préservés).
            using var req = UnityWebRequest.Get($"{relay}/register");
