...
namespace MiSideCoop.Avatars
{
    ...
    public class Player2Avatar : PlayerAvatar
    {
        ...
        {
            // ── v1.3.3 IL2CPP-SAFE WRAP ──
            // En IL2CPP MiSide, plusieurs API génériques de UnityEngine sont
            // strippées (GetComponentsInChildren<T>, etc.). Une exception ici
            // remonterait dans le trampoline IL2CPP-to-Managed et casserait
            // SpawnPlayer2Remote/Local → le guest reste bloqué sur CHARGEMENT.
            // On enveloppe TOUTE l'init dans un try/catch pour garantir que
            // le pipeline réseau continue même si l'avatar n'est pas rendu.
            try
            {
            ...
            catch (Exception ex)
            {
                MiSideCoopPlugin.Logger?.LogError(
                    $"[Co-op] Player2Avatar.Initialize('{playerName}', local={isLocal}) failed: {ex.Message}\n{ex.StackTrace}");
                // On laisse PlayerName/IsLocalPlayer renseignés par base.Initialize
                // si possible, pour que le syncing réseau puisse continuer même
                // sans rendu visuel correct.
            }
        }
