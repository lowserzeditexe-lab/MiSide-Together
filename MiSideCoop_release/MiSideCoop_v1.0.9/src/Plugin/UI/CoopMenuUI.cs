...
namespace MiSideCoop.UI
{
    ...
    public class CoopMenuUI : MonoBehaviour
    {
        ...
        /// <summary>
        /// Parcourt la scène active pour récupérer une Font utilisée par un Text
        /// existant. Le jeu MiSide utilise uGUI pour ses menus → il y a forcément
        /// des Text avec une font valide quelque part.
        /// </summary>
        private static Font TryFindFontInScene()
        {
            try
            {
                ...
                {
                    var f = ScanTransformForFont(roots[i].transform, 0);
                    if (f != null) return f;
                }
            }
            catch (Exception ex)
            {
                MiSideCoopPlugin.Logger?.LogWarning($"[Co-op] TryFindFontInScene : {ex.Message}");
            }
            return null;
        }
